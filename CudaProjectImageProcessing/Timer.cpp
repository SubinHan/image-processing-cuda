...
float Timer::get_elapsed_seconds()
{
    ...
    timeval gap;
    get_gap_time(&start, &now, &gap);

    return timeval_to_float(&gap);
}
...
int Timer::get_time_of_day(timeval* tv, timezone* tz)
{
    if (tv) {
        FILETIME               filetime; /* 64-bit value representing the number of 100-nanosecond intervals since January 1, 1601 00:00 UTC */
        ULARGE_INTEGER         x;
...
float Timer::timeval_to_float(timeval* time)
{
    float val;
    val = time->tv_sec;
    val += (time->tv_usec * 0.000001);
...
void Timer::get_gap_time(timeval* start_time, timeval* end_time, timeval* gap_time)
{
    gap_time->tv_sec = end_time->tv_sec - start_time->tv_sec;
    gap_time->tv_usec = end_time->tv_usec - start_time->tv_usec;
    if (gap_time->tv_usec < 0) {
